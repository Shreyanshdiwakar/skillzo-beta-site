...
export default function Navbar() {
  ...
  return (
    ...
    >
            ...
            <HoverCard key={link.name} openDelay={100} closeDelay={100}>
              <HoverCardTrigger asChild>
                <span className="hover:text-yellow-400 transition flex items-center gap-1 cursor-pointer select-none">
                  {link.name}
                  <svg className="w-4 h-4 ml-0.5 transition-transform group-hover:rotate-180" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <Link 
        href="/joincommunity" 
        className="hidden md:inline bg-primary text-black px-6 py-3 rounded-full font-bold text-lg shadow hover:bg-yellow-500 transition"
      >
        Join Skillzo
      </Link>
          ...
          {navLinks.map((link, idx) =>
            link.dropdown ? (
              <div key={link.name} className="w-full flex flex-col items-center">
                ...
                >
                  <span>{link.name}</span>
                  <svg className={`w-4 h-4 transition-transform ${openDropdown === idx ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {openDropdown === idx && (
